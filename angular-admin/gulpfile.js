...
function renderHtml (isProd) {
    ...
    function renderHtml (tpl, data) {
        ...
        fs.readFile(path.join(__dirname, 'src/page', data.fileName), function (err, pageHtmlSrc) {
            if (err) {
                return console.error(err)
            }

            tpl = ejs.render(tpl, {
                view: '<div ng-controller="' + pageName + '" class="' + config.pageClassName + ' ' + ('page-' + pageName) + '">' + pageHtmlSrc.toString() + '</div>',
                base: '/' + publicPath[1] + '/',
                page: pageName,
